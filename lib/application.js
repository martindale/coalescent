...
Application.prototype.connect = function() {
  ...
  socket.on('end', function() {
    self.connections.outbound.forEach(function(sock, i) {
      if (sock === socket) {
        self.connections.outbound.splice(i, 1);
      }
    });
  });
...
Application.prototype._setupRelay = function(outSock) {
  var self = this;
  // if relaying messages, then pipe inbound to outbound
  if (self.options.relayMessages) {
    self.connections.inbound.forEach(function(inSock) {
      ...
      inSock.pipe(outSock).pipe(inSock);
    });
  }
};
...
Application.prototype._handleInbound = function(socket) {
  ...
  socket.on('end', function() {
    self.connections.inbound.forEach(function(sock, i) {
      if (sock === socket) {
        self.connections.inbound.splice(i, 1);
      }
    });
  });
